#ifndef DEEPX_MEM_TENSORMAP_HPP
#define DEEPX_MEM_TENSORMAP_HPP
...
namespace deepx::mem
{
    ...
    public:
        ...
        // 拷贝构造函数
        TensorMap(const TensorMap& other) : mem(other.mem) {
            // shared_ptr 会自动处理引用计数
        }

        // 移动构造函数
        TensorMap(TensorMap&& other) noexcept : mem(std::move(other.mem)) {
            // 移动后 other.mem 为空
        }
        ...
        // 析构函数
        ~TensorMap() = default; // shared_ptr 会自动处理内存释放
